/**
 * Script to load historical dynamic energy prices
 * 
 * Loads prices from 2020-11-30 to 2025-11-30 (5 years)
 * Uses EnergyZero API (primary) and ENTSOE (fallback)
 * 
 * Usage:
 *   npx ts-node scripts/load-historical-dynamic-prices.ts
 * 
 * Or with date range:
...
interface EnergyPrices {
  ...
  source: 'ENERGYZERO' | 'ENTSOE' | 'FALLBACK' | 'MANUAL'
  date: string
}
